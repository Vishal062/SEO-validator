    ...
    <div className="overflow-x-auto">
      {Array.isArray(schema) && schema.length > 0 ? (
        <table className="w-full text-xs border-separate border-spacing-y-1">
          <tbody>
            {schema.map((obj) => renderRows(obj, 0)).filter(Boolean)}
          </tbody>
        </table>
      ) : (
          ...
          {ogKeys.map(key => (
            <div key={key}>
              ...
              <div className={og[key] ? 'text-gray-900' : 'text-red-600 font-semibold'}>
                {typeof og[key] === 'string' && og[key] ? og[key] : 'Missing'}
              </div>
            </div>
          ))}
        </div>
          ...
          {twitterKeys.map(key => (
            <div key={key}>
              ...
              <div className={twitter[key] ? 'text-gray-900' : 'text-red-600 font-semibold'}>
                {typeof twitter[key] === 'string' && twitter[key] ? twitter[key] : 'Missing'}
              </div>
            </div>
          ))}
        </div>
...
export default function CompareSeoResultBox({ uat, prod }: CompareSeoResultBoxProps) {
  ...
  // Error handling
  if ((uat as Record<string, unknown>).error || (prod as Record<string, unknown>).error) {
    return (
      <div className="flex gap-4">
        {typeof (uat as Record<string, unknown>).error === 'string' && (uat as Record<string, unknown>).error ? (
          <div className="flex-1 p-6 bg-red-50 border border-red-300 rounded-2xl shadow-lg flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-base text-red-600 font-semibold">Error: {(uat as Record<string, unknown>).error as string}</p>
            </div>
          </div>
        ) : null}
        {typeof (prod as Record<string, unknown>).error === 'string' && (prod as Record<string, unknown>).error ? (
          <div className="flex-1 p-6 bg-red-50 border border-red-300 rounded-2xl shadow-lg flex items-start gap-4">
            <div className="text-red-500 text-2xl mt-1">❌</div>
            <div>
              <h3 className="text-lg font-bold text-red-700 break-all">PROD</h3>
              <p className="mt-1 text-base text-red-600 font-semibold">Error: {(prod as Record<string, unknown>).error as string}</p>
            </div>
          </div>
        ) : null}
      </div>
    ...
    <div className="p-4 bg-white rounded-2xl shadow-lg space-y-6 text-xs md:text-sm w-full">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              ...
              {(() => {
                ...
                return fields.map(({ key, label, icon, type }) => {
                  const uVal = (uat as Record<string, unknown>)[key];
                  const pVal = (prod as Record<string, unknown>)[key];
      ...
      {tab === 'Headings' && (
        <div className="flex flex-col md:flex-row gap-2 md:gap-4">
          <div className="flex-1 w-full space-y-2">
            <div className="font-bold text-purple-700 mb-2">UAT</div>
            {(() => {
              const uHeadings: Array<{ level: string; text: string }> = Array.isArray((uat as Record<string, unknown>).headings) ? (uat as Record<string, Array<{ level: string; text: string }> >).headings : [];
              const pHeadings: Array<{ level: string; text: string }> = Array.isArray((prod as Record<string, unknown>).headings) ? (prod as Record<string, Array<{ level: string; text: string }> >).headings : [];
              const maxLen = Math.max(uHeadings.length, pHeadings.length);
              if (!maxLen) return null;
              return Array.from({ length: maxLen }).map((_, idx) => {
                const u = uHeadings[idx];
                const p = pHeadings[idx];
                if (!u && !p) return null;
                const isMissing = !u;
              ...
              }).filter(x => x && typeof x === 'object' && 'type' in x);
            })()}
          </div>
          <div className="flex-1 w-full space-y-2">
            <div className="font-bold text-blue-700 mb-2">PROD</div>
      ...
      {tab === 'Links' && (
        <div className="flex flex-col gap-2">
          ...
          {(() => {
            const uLinks: Array<{ href?: string; anchor: string }> = Array.isArray((uat as Record<string, unknown>).links) ? (uat as Record<string, Array<{ href?: string; anchor: string }> >).links : [];
            const pLinks: Array<{ href?: string; anchor: string }> = Array.isArray((prod as Record<string, unknown>).links) ? (prod as Record<string, Array<{ href?: string; anchor: string }> >).links : [];
            const maxLen = Math.max(uLinks.length, pLinks.length);
            if (!maxLen) return null;
            return Array.from({ length: maxLen }).map((_, idx) => {
              const u = uLinks[idx];
              const p = pLinks[idx];
              ...
              const anchorMissing = !u || !u.anchor;
              const anchorDiff = u && p && u.anchor !== p.anchor;
              const pathMissing = !u || !u.href;
              ...
              return (
                <React.Fragment key={idx}>
                  ...
                  <div className="bg-gray-50 rounded-lg p-2 mb-1 border border-gray-200">
                    {p ? (
                      <>
                        <div className={(!p || !p.href) ? 'font-bold text-red-600 break-all whitespace-pre-line text-xs' : (u && pPath !== uPath) ? 'font-bold text-orange-500 break-all whitespace-pre-line text-xs' : 'font-bold text-black break-all whitespace-pre-line text-xs'}>
                          {p.href || 'Undefined (No href attribute)'}
                        </div>
                        <div className="text-base break-all whitespace-pre-line text-xs">
                          <span className="font-bold">Anchor:</span> <span className={(!p || !p.anchor) ? 'text-red-600 font-semibold' : (u && p.anchor !== u.anchor) ? 'text-orange-500 font-semibold' : 'text-black'}>{p.anchor || 'Missing Anchor'}</span>
                        </div>
                      </>
                    ) : null}
                  </div>
      ...
      {tab === 'Schema' && (
        <div className="flex flex-col md:flex-row gap-2 md:gap-4">
          <div className="flex-1 w-full overflow-x-auto">
            <SchemaTable schema={Array.isArray((uat as Record<string, unknown>).schema) ? (uat as Record<string, unknown>).schema as unknown[] : []} />
          </div>
          <div className="flex-1 w-full overflow-x-auto">
            <SchemaTable schema={Array.isArray((prod as Record<string, unknown>).schema) ? (prod as Record<string, unknown>).schema as unknown[] : []} />
          </div>
        </div>
      )}
      {tab === 'Social' && (
        <div className="flex flex-col md:flex-row gap-2 md:gap-4">
          <div className="flex-1 w-full">
            <SocialFields data={uat as Record<string, unknown>} />
          </div>
          <div className="flex-1 w-full">
            <SocialFields data={prod as Record<string, unknown>} />
          </div>
        </div>
      )}
    </div>
